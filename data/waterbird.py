...
class Waterbird(Dataset):
    def __init__(self,
        ...
        if split=='train':
            # Benchmark cmnist
            self.align = glob.glob(os.path.join(root_path, 'benchmarks', 'waterbird', conflict_ratio+'pct', 'align', '*', '*')) # for each *, class_idx and image_id
            self.conflict = glob.glob(os.path.join(root_path, 'benchmarks', 'waterbird', conflict_ratio+'pct', 'conflict', '*', '*'))
            self.data = self.align + self.conflict
        
        elif split=='valid':
            # Benchmark cmnist
            self.data = glob.glob(os.path.join(root_path, 'benchmarks', 'waterbird', 'valid', '*')) # *: image_id

        elif split=='test':
            # Benchmark cmnist
            self.data = glob.glob(os.path.join(root_path, 'benchmarks', 'waterbird', 'test', '*')) # *: image_id
